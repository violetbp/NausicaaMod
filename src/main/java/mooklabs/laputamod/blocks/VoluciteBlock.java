...
public class VoluciteBlock extends Block {
	...
	public boolean isConstructedCorrectly(World w, int x, int y, int z) {
		for (int i = -1; i < 2; i++)
			for (int j = -1; j < 2; j++)
				if (!isBlckRght(w, x + i, y - 1, z + j, Blocks.gold_block)) return false;
		for (int i = -2; i < 3; i++)
			for (int j = -2; j < 3; j++)
				if (!isBlckRght(w, x + i, y - 2, z + j, Blocks.stonebrick)) return false;

		return true;
	}

	public boolean isBlckRght(World w, int x, int y, int z, Block n) {
